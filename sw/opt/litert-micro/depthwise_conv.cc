...
void DepthwiseConvPerChannelPatch(
  ...
  auto* accs = reinterpret_cast<int32_t*>(
      aligned_alloc(16, sizeof(int32_t) * (out_x_ed - out_x_st) * out_d));
...
void DepthwiseConvPerChannelPatchCenter3x3Reuse6(
    ...
    int out_x_st, int out_x_ed) {
  // Get parameters.
  // TODO: DCHECK stride_w == dilation_w
  const int stride_w = params.stride_width;
  const int stride_h = params.stride_height;
  const int dilation_w = params.dilation_width_factor;
  const int dilation_h = params.dilation_height_factor;
  ...
  auto* accs = reinterpret_cast<int32_t*>(
      aligned_alloc(16, sizeof(int32_t) * out_patch_h * out_patch_w * out_d));
  TFLITE_DCHECK_NE(accs, nullptr);
...
void DepthwiseConvPerChannel(
  ...
  auto* f_data_copy = reinterpret_cast<int8_t*>(
      aligned_alloc(16, sizeof(int8_t) * f_shape.FlatSize()));
  TFLITE_DCHECK_NE(f_data_copy, nullptr);  // TODO:
  Memcpy(f_data_copy, f_data, sizeof(int8_t) * f_shape.FlatSize());
  int32_t* bias_data_copy = nullptr;
  if (bias_data) {
    bias_data_copy =
        reinterpret_cast<int32_t*>(aligned_alloc(16, sizeof(int32_t) * out_d));
    TFLITE_DCHECK_NE(bias_data_copy, nullptr);  // TODO:
    Memcpy(bias_data_copy, bias_data, sizeof(int32_t) * out_d);
  }

  // Shifting from quantization params.
  auto* shift_left =
      reinterpret_cast<uint8_t*>(aligned_alloc(16, sizeof(uint8_t) * out_d));
  TFLITE_DCHECK_NE(shift_left, nullptr);
  auto* shift_right =
      reinterpret_cast<uint8_t*>(aligned_alloc(16, sizeof(uint8_t) * out_d));
  TFLITE_DCHECK_NE(shift_right, nullptr);
  PrepareShiftParams(shift_left, shift_right, output_shift, out_d);
  ...
  // Top
  DepthwiseConvPerChannelPatch(params, output_multiplier, shift_left,
                               shift_right, in_shape, in_data, f_shape,
                               f_data_copy, bias_shape, bias_data_copy,
                               out_shape, out_data, 0, out_y_top, 0, out_w);
  // Middle-left
  DepthwiseConvPerChannelPatch(
      params, output_multiplier, shift_left, shift_right, in_shape, in_data,
      f_shape, f_data_copy, bias_shape, bias_data_copy, out_shape, out_data,
      out_y_top, out_y_bottom, 0, out_x_left);
  // Center
  if ((f_h == 3) && (f_w == 3)) {
    if (stride_w == dilation_w) {
      DepthwiseConvPerChannelPatchCenter3x3Reuse6(
          params, output_multiplier, shift_left, shift_right, in_shape, in_data,
          f_shape, f_data_copy, bias_shape, bias_data_copy, out_shape, out_data,
          out_y_top, out_y_bottom, out_x_left, out_x_right);
      goto center_done;
    }
...
center_done:
  ...
  DepthwiseConvPerChannelPatch(
      params, output_multiplier, shift_left, shift_right, in_shape, in_data,
      f_shape, f_data_copy, bias_shape, bias_data_copy, out_shape, out_data,
      out_y_top, out_y_bottom, out_x_right, out_w);
  // Bottom
  DepthwiseConvPerChannelPatch(
      params, output_multiplier, shift_left, shift_right, in_shape, in_data,
      f_shape, f_data_copy, bias_shape, bias_data_copy, out_shape, out_data,
      out_y_bottom, out_h, 0, out_w);
