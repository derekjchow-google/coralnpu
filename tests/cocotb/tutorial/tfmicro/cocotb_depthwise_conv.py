...
class DepthwiseConvTest:
    ...
    def __init__(self, in_d, dm, stride, out_h = 4, out_w = 4):
        self.dm = dm
        self.stride = stride
        out_d = in_d * dm
        in_h = out_h * stride
...
@cocotb.test()
async def test_dwconv8to8(dut):
    t = DepthwiseConvTest(in_d=8, dm=1, stride=1)
    await t.load_and_populate_input(dut)
    await t.test(ref_target=226_000, opt_target=26_600)

@cocotb.test()
async def test_dwconv8to8stride2(dut):
    t = DepthwiseConvTest(in_d=8, dm=1, stride=2)
    await t.load_and_populate_input(dut)
    await t.test(ref_target=257_000, opt_target=26_400)


@cocotb.test()
async def test_dwconv32to32(dut):
    t = DepthwiseConvTest(in_d=32, dm=1, stride=1)
    await t.load_and_populate_input(dut)
...
async def test_dwconv32to32stride2(dut):
    t = DepthwiseConvTest(in_d=32, dm=1, stride=2)
    await t.load_and_populate_input(dut)
    await t.test(ref_target=1_019_000, opt_target=28_500)


@cocotb.test()
async def test_dwconv64to64(dut):
    t = DepthwiseConvTest(in_d=64, dm=1, stride=1)
    await t.load_and_populate_input(dut)
...
async def test_dwconv64to64stride2(dut):
    t = DepthwiseConvTest(in_d=64, dm=1, stride=2)
    await t.load_and_populate_input(dut)
...
async def benchmark_dwconv8to8(dut):
    t = DepthwiseConvTest(in_d=8, dm=1, stride=1, out_h=112, out_w=112)
    await t.load_and_populate_input(dut)
...
async def benchmark_dwconv32to32(dut):
    t = DepthwiseConvTest(in_d=32, dm=1, stride=1, out_h=56, out_w=56)
    await t.load_and_populate_input(dut)
...
async def benchmark_dwconv64to64(dut):
    t = DepthwiseConvTest(in_d=64, dm=1, stride=1, out_h=28, out_w=28)
    await t.load_and_populate_input(dut)
...
async def benchmark_dwconv128to128(dut):
    t = DepthwiseConvTest(in_d=128, dm=1, stride=1, out_h=14, out_w=14)
    await t.load_and_populate_input(dut)
...
async def benchmark_dwconv256to256(dut):
    t = DepthwiseConvTest(in_d=256, dm=1, stride=1, out_h=7, out_w=7)
    await t.load_and_populate_input(dut)
